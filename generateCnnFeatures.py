...
json_file = open('models/dmlvh2_mLSTM_1024_2_model.json', 'r')
model = json_file.read()
json_file.close()
model = model_from_json(model)
model.load_weights("models/dmlvh2_mLSTM_1024_2_weights.h5")
model = Model(inputs=model.input, outputs=model.get_layer('dense_1').output) # dense_2 for features , dense_3 for predictions

X = []
X = np.load(open("NP2.npy"))
X.shape
print(X.shape[:])
...
np.savetxt("hashCodes/dmlvh2_mLSTM_Features_1024_2.txt", features, fmt='%f')
