...
Y = pd.read_csv(r'/home/ubuntu/Desktop/Thesis_Follow_Up_3/Datasets/CPSM/Y2.csv') # Video files labels, one hot encoded
Y.shape
print(Y.shape[:])


X = np.load(open('NP.npy')) # Join video frames in to one NPY file , see deneme.py
X.shape
...
lstm    = LSTM(32 , input_shape=(X.shape[1], X.shape[2]), return_sequences = False )(visible) 
Dense_1 = Dense(2048, activation='relu')(lstm)
Dense_2 = Dense(hash_bits, activation='sigmoid')(Dense_1)
Dense_3 = Dense(6, activation='sigmoid')(Dense_2)
...
def c_loss(noise_1, noise_2):
    def loss(y_true, y_pred):
        return (K.categorical_crossentropy(y_true, y_pred) + (1/hash_bits) * (K.sum((noise_1 - noise_2)**2) )) 
...
with open("models/test_64_model.json", "w") as json_file:
    json_file.write(model_json)
model.save_weights("models/test_64_weights.h5")
